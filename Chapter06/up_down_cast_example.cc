...
int main(int argc, char* argv[])
{
    ...
    //D& dref2= dynamic_cast<D&>(b); //Throw bad_cast
    ...
    C* cdp= dynamic_cast<C*>(bdp); // correct-cast from B to C ok: bdp points to an object of type D
    ...
    F* fdp= dynamic_cast<F*>(bdp); // cast from B to F, already clear at compiler time that it is nonsense but not blamed
